...
Việc tính toán độ rộng bit cho các thanh ghi trễ được tính toán theo công thức \ref{bitcic}. Với số tầng là 4 và hệ số Decimation 12x ta có độ rộng bit như sau:
\begin{equation}
...
Bảng \ref{cic_top_t} liệt kê các cổng vào ra của khối dti\_cic\_decimator. 

Như chúng ta đã giới thiệu ở mục \ref{cic_filter_ref}, bộ lọc CIC sẽ chia ra các phần "comb" và "integrator". Hai khối này sẽ hoạt động với 2 tần số khác nhau ứng với tỷ lệ Decimation. Với số lượng các "comb" và "integrator" sẽ bằng số tầng của CIC. Mỗi "comb" sẽ sử dụng 1 bộ trừ, còn "integrator" là 1 bộ cộng. Như vậy, kiến trúc CIC 4 tầng đơn giản phải sử dụng đến 4 bộ cộng và 4 bộ trừ. Với kiến trúc này (hình \ref{cic_top_arc}), chúng ta sẽ gom chúng lại và sử dụng tần số gấp 4 lần tần số chúng hoạt động. Như vậy, chúng ta chỉ cần sử dụng 1 bộ cộng và 1 bộ trừ cho bộ lọc CIC. Lúc này, các bộ \textbf{counter} sẽ có nhiệm vụ chọn thành phần "comb" và "integrator" sử dụng bộ cộng và trừ sao cho đúng thời điểm. Thứ tự hoạt động sẽ dựa vào vị trí của thành phần đó tính từ đầu vào đến đầu ra.
...
Ta thấy, các điểm tần số chuyển tiếp giữa các vùng (dải thông, dải chuyển tiếp, dải dừng) giữa 2 tín hiệu là giống nhau. Tuy nhiên, khi chúng ta tập trung vào dải dừng, với phần mềm độ suy hao ở dải đó cao nhất là 146.721 dB, còn phần cứng là 134.789, chênh lệch nhau 11.932 dB. 
...
\begin{figure}[H]
    ...
    \caption[Biểu đồ phổ của âm thanh sau khi chuyển đổi bằng phần mềm]{\bfseries \fontsize{12pt}{0pt}\selectfont  Biểu đồ phổ của âm thanh sau khi chuyển đổi bằng phần mềm}
    \label{vldd2_psd_1}
\end{figure}
